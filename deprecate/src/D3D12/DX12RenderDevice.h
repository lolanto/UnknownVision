...
private:
	...
	public:
		...
		void* GetResource() override final { assert(m_pBackBuffer != nullptr); return m_pBackBuffer; }
...
public:
	...
	Texture2D* BackBuffer() override final { return &m_swapChainResources[m_curBackBufferIndex]; }
	Buffer* CreateBuffer(size_t capacity, size_t elementStride, ResourceStatus status) override final;
	virtual Texture2D* CreateTexture2D(size_t width, size_t height, size_t miplevels, ElementFormatType format, ResourceStatus status) override final;
	bool WriteToBuffer(void* pSrc, Buffer* pDest, size_t srcSize, size_t destOffset, CommandUnit* cmdUnit) override final;
	bool WriteToTexture2D(Image* pSrc, Texture2D* pDest, CommandUnit* cmdUnit) override final;
