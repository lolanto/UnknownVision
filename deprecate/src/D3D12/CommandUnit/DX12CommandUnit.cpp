...
size_t DX12CommandUnit::Flush(bool bWaitForCompletion)
{
	...
	assert(SUCCEEDED(m_graphicsCmdList->Reset(m_pAllocator, nullptr)));
...
{
	if (pResource->m_status.isStably() == false) return;
...
{
	assert(pMgr != nullptr && graphicsCmdList != nullptr && allocator != nullptr);
	assert(m_pMgr == nullptr && m_graphicsCmdList == nullptr && m_pAllocator == nullptr);
	m_pMgr = pMgr;
...
bool DX12CommandUnit::Reset(ID3D12CommandAllocator * allocator)
{
	...
	assert(m_pMgr != nullptr && m_graphicsCmdList != nullptr);
	m_pAllocator = allocator;
