...
public:
	...
	virtual ~DX12StaticIndexBuffer() = default;
...
std::unique_ptr<StaticIndexBuffer> StaticIndexBuffer::Create(size_t count, void* pData) {
	std::unique_ptr<StaticIndexBuffer> newResource = std::make_unique<DX12StaticIndexBuffer>();
	newResource->SetInitialData(count, pData);
	return newResource;
}
