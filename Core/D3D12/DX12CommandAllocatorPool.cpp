...
void CommandAllocatorPool::Shutdown()
{
    ...
    m_ReadyAllocators = decltype(m_ReadyAllocators)();
}
...
ID3D12CommandAllocator * CommandAllocatorPool::RequestAllocator(uint64_t CompletedFenceValue)
{
    ...
    if (!m_ReadyAllocators.empty())
    {
        ...
        if (AllocatorPair.first <= CompletedFenceValue)
        {
            ...
            if (FAILED(pAllocator->Reset())) {
                LOG_ERROR("Command Allocator reset failed!");
                abort();
            }
    ...
    if (pAllocator == nullptr)
    {
        ...
        if (FAILED(m_Device->CreateCommandAllocator(m_cCommandListType, IID_PPV_ARGS(&tmpAllocator)))) {
            LOG_ERROR("Create Command Allocator failed!");
            abort();
        }
