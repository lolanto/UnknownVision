...
public:
	DX12ResourceManager2(ID3D12Device* device) 
		: m_pdevice(device), m_pallocator(nullptr) {}
	/** 禁止非右值传递 */
	~DX12ResourceManager2() = default; /**< 假如有资源正在被引用，可能会造成错误 */
	DX12ResourceManager2(const DX12ResourceManager2&) = delete;
	DX12ResourceManager2(DX12ResourceManager2&& rhs);
	DX12ResourceManager2& operator=(const DX12ResourceManager2&) = delete;
	DX12ResourceManager2& operator=(DX12ResourceManager2&&);
 ...
 * @param state 之前请求的纹理当前的状态
 * @return 归还成功返回true, 失败返回false */
	bool ReleaseResource(ID3D12Resource* D3D12_RESOURCE_STATES) thread_safe;
private:
