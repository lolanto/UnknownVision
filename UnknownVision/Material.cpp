...
void Material::Bind(ID3D11DeviceContext* devCtx) {
	...
	if (m_pixelShader) m_pixelShader->Bind(devCtx);
	...
	for (auto iter = m_texStates.begin(), end = m_texStates.end(); iter != end; ++iter) {
		...
		case SBT_GEOMETRY_SHADER:
			devCtx->GSSetShaderResources(iter->slot, 1, iter->texture->GetSRV());
		default:
			break;
		}
	}
...
void Material::Unbind(ID3D11DeviceContext* devCtx) {
	...
	if (m_pixelShader) m_pixelShader->Unbind(devCtx);
	...
	for (auto iter = m_texStates.begin(), end = m_texStates.end(); iter != end; ++iter) {
		...
		case SBT_PIXEL_SHADER:
			...
			break;
		case SBT_GEOMETRY_SHADER:
			devCtx->GSSetShaderResources(iter->slot, 1, NULLPTR);
			break;
		default:
			break;
		}
