...
float3 LTC_Evaluate(
  ...
  if (isDif) {
    ...
    difCol = base.Sample(linearSampler, tuv).xyz;
  }
...
float4 main(VSOutput i) : SV_Target {
  ...
  float3 dcol = float3(2.0, 2.0, 2.0); // 灯光diffuse颜色
  float3 scol = float3(0.8, 0.8, 0.8); // 灯光specular颜色
  ...
  if (hitFloor) {
    ...
    float2 uv = float2(0.35, theta / (0.5 * pi));
    // uv.y = 1.0f - uv.y;
    ...
    uvChange(tuv.x);
    uvChange(tuv.y);
    float level = distToRect * distToRect * 0.5f;
    float3 res = base.SampleLevel(linearSampler, tuv, level).xyz;
    // uint2 min_max;
