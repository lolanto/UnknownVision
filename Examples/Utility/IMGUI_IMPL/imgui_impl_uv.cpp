...
OutputStageOptions IMGUI_OUTPUT_STAGE_FUNC() {
	...
	options.rtvFormats[0] = BackBufferFormat;
...
void ImGui_ImplUV_CreateFontsTexture(RenderDevice* pDev) {
	...
	std::unique_ptr<MImage::Image> fontTex = MImage::Image::LoadImageFromMemory(pixelData, width, height, MImage::IMAGE_FORMAT_R8G8B8A8);
	/** 向GPU上传纹理资源 */
	gFontTexture.reset(pDev->CreateTexture2D(width, height, 1, 1, ELEMENT_FORMAT_TYPE_R8G8B8A8_UNORM, ResourceStatus(RESOURCE_USAGE_SHADER_RESOURCE, RESOURCE_FLAG_STABLY)));
	if (gFontTexture == nullptr) abort();
	auto cmdUnit = pDev->RequestCommandUnit(DEFAULT_COMMAND_UNIT);
	ImageDesc desc;
	desc.width = fontTex->Width();
	desc.height = fontTex->Height();
	desc.rowPitch = fontTex->GetRowPitch(0);
	desc.slicePitch = fontTex->GetSlicePitch(0);
	desc.data = fontTex->GetData(0);
	if (pDev->WriteToTexture2D({ desc }, gFontTexture.get(), cmdUnit) == false) abort();
...
IMGUI_IMPL_API bool ImGui_ImplUV_Init(RenderDevice* pDev, RenderBackend* pBackend, size_t reserveFrames) {
	...
	if (pBackend->InitializeShaderObject(gvs.get()) == false) abort();
	if (pBackend->InitializeShaderObject(gps.get()) == false) abort();
